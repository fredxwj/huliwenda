...
<html>
  ...
  <body>
    ...
    <div id="container">
		...
		<nav id="nav">
		  <ul>
			<li><a href="<?php echo U('mobile/index/init');?>" class="nav_current">网站首页</a></li>
		...
		<div class="topPic">
			<div class="imgSlideMain">
				...
				<li><a href="<?php echo U('mobile/index/show', array('catid'=>$v['catid'],'id'=>$v['id']));?>"><img src="<?php echo $v['thumb'];?>" alt="<?php echo $v['title'];?>" title="<?php echo $v['title'];?>"></a><p><?php echo $v['title'];?></p></li>
				<?php } ?>							
				</ul>
				</div>
		...
		<div id="main">	
		 <div class="list_news">
		  ...
		  <?php $tag = yzm_base::load_sys_class('yzm_tag');if(method_exists($tag, 'lists')) {$data = $tag->lists(array('field'=>'id,catid,title,url','modelid'=>'1','limit'=>'20',));}?>
		   <?php if(is_array($data)) foreach($data as $v) { ?>
		   <li><a href="<?php echo U('mobile/index/show', array('catid'=>$v['catid'],'id'=>$v['id']));?>" title="<?php echo $v['title'];?>"><?php echo $v['title'];?></a></li>
		  <?php } ?>		  	 
		  </ul>		 
		 </div>
		 <div class="list_news">
		  ...
		  <ul>
		   <!-- 这里只做演示，选择modelid为1的最近更新，您可以根据自己的需求进行更改 -->
		  <?php $tag = yzm_base::load_sys_class('yzm_tag');if(method_exists($tag, 'lists')) {$data = $tag->lists(array('field'=>'id,catid,title,inputtime','modelid'=>'1','flag'=>'3','limit'=>'5',));}?>
		   <?php if(is_array($data)) foreach($data as $v) { ?>
		   <li><a href="<?php echo U('mobile/index/show', array('catid'=>$v['catid'],'id'=>$v['id']));?>" title="<?php echo $v['title'];?>"><?php echo $v['title'];?></a></li>
		  <?php } ?>		  	 
		  </ul>		 
		 ...
		 <div class="img_news">
		  ...
		  <ul>
		   <!-- 这里只做演示，选择modelid为1的最近更新，您可以根据自己的需求进行更改 -->
			...
			<li><a href="<?php echo U('mobile/index/show', array('catid'=>$v['catid'],'id'=>$v['id']));?>"><img src="<?php echo $v['thumb'];?>" alt="<?php echo $v['title'];?>" title="<?php echo $v['title'];?>"><p><?php echo $v['title'];?></p></a></li>
			<?php } ?>			  			 
		  </ul>		 
		 </div>	 		 
		</div>
